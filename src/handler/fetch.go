...
// It communicates with all other systems via channels
// ([]byte channel and error channel).
func HandleFetch(token string, start, end time.Time, chbs chan []byte, cher chan error) (*entities.QueryPunchResponse, error) {
	query := new(entities.QueryPunchResponse)
