...
// passed.
func NewFetchPunch(c restclient.Client) repositories.FetchPunchClient {
	if c == nil {
...
func (f *fetchPunch) GetPunchByID(token, punchID string) (*entities.PunchResponse, error) {
	uri := fmt.Sprintf("%s/%s", constant.SitePunchURI, punchID)
	headers := map[string]string{
		"cookie": fmt.Sprintf("qwert-external=%s", token),
	}

	resp := new(entities.PunchResponse)
...
func (f *fetchPunch) GetPunchByDateRange(token string, start, end time.Time) (*entities.QueryPunchResponse, error) {
	headers := map[string]string{
		"cookie": fmt.Sprintf("qwert-external=%s", token),
	}
	uri := fmt.Sprintf("%s?inicio=%s&fim=%s", constant.SitePunchURI, start.Format(constant.DateLayout), end.Format(constant.DateLayout))

	resp := new(entities.QueryPunchResponse)
	if err := f.client.Get(uri, headers, resp); err != nil {
		return nil, err
	}
